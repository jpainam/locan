...
session_start();
	...
	*/
	...
	function get_friend_list($userid, $time)
	{
		...
		");
		*/
		$sql = "SELECT IDUSER userid, LOGIN username, arrowchat_status.session_time lastactivity, AVATAR avatar, IDUSER link, 
		arrowchat_status.is_admin, arrowchat_status.status 
		FROM users 
		LEFT JOIN arrowchat_status ON users.LOGIN = arrowchat_status.userid  
		WHERE arrowchat_status.session_time + 60 + ". $online_timeout. " > " . time() . " AND users.LOGIN != '".$userid."' 
