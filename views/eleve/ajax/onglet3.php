...
for ($i = 0; $i < $t = count($enseignements); $i++) {
    ...
    if(!empty($heurfin) && !empty($heurdeb)){
        echo "<script>addElement(\"" . $enseignements[$i]["LIBELLE"] . "\",\"" . $enseignements[$i]["CIVILITE"]." ".$enseignements[$i]["NOM"]."<br/>".$enseignements[$i]['PRENOM'] . "\"," . $enseignements[$i]["JOUR"]  . ",\"" . $heurdeb . "\",\"" . $heurfin . "\");</script>";
        $heurdeb = "";
        $heurfin = "";
    }
}
